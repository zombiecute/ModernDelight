    ...
    private void putItem(World world) {
        ...
        for (int i = 9;i<12;i++){
            if (this.getStack(i).isEmpty()){
                setStack(i,getDigestate().getDefaultStack());
                hasSpawn = true;
                break;
            } else if (this.getStack(i).getItem().equals(getDigestate())){
                int count = this.getStack(i).getCount();
                if (count < this.getStack(i).getMaxCount()){
                    setStack(i,new ItemStack(getDigestate(),count+1));
                    hasSpawn = true;
                    break;
                }
            }
        ...
        if (!hasSpawn){
            ItemScatterer.spawn(world,pos.getX(), pos.getY()+1, pos.getZ(),getDigestate().getDefaultStack());
        }
    }
