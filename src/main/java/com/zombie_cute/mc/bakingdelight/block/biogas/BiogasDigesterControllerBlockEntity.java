...
public class BiogasDigesterControllerBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory<BlockPos> {
    ...
    private int time = 60;
    public static void tick(World world, BlockPos pos, BlockState state, BiogasDigesterControllerBlockEntity b) {
        if (world.isClient){
            return;
        }
        b.time--;
        ...
        if (b.checked==1){
            b.maxGasValue = b.size * 1000;
            b.markDirty();
        } else {
            b.maxGasValue = 0;
            b.markDirty();
        }
        if (b.gasValue >= Short.MAX_VALUE){
            b.shortGasValue = b.gasValue/19;
            b.isSplit = 1;
        } else {
            b.shortGasValue = b.gasValue;
            b.isSplit = 0;
        }
        if (b.gasValue > b.maxGasValue && b.time == 1){
            if (b.gasValue >= 1000){
                b.createExplode(world, pos);
            } else {
                if (b.gasValue > 500){
    ...
    public void createExplode(World world, BlockPos pos) {
        if (gasValue < 6000){
            world.setBlockState(pos.down(),ModBlocks.LIQUEFIED_BIOGAS_FLUID_BLOCK.getDefaultState());
            world.setBlockState(pos.down(2),ModBlocks.LIQUEFIED_BIOGAS_FLUID_BLOCK.getDefaultState());
            world.createExplosion(null, pos.getX(), pos.getY(), pos.getZ(),3.5f,true, World.ExplosionSourceType.BLOCK);
        } else {
            world.setBlockState(pos.west(),ModBlocks.LIQUEFIED_BIOGAS_FLUID_BLOCK.getDefaultState());
            world.setBlockState(pos.east(),ModBlocks.LIQUEFIED_BIOGAS_FLUID_BLOCK.getDefaultState());
            world.setBlockState(pos.north(),ModBlocks.LIQUEFIED_BIOGAS_FLUID_BLOCK.getDefaultState());
            world.setBlockState(pos.south(),ModBlocks.LIQUEFIED_BIOGAS_FLUID_BLOCK.getDefaultState());
            world.setBlockState(pos.up(),ModBlocks.LIQUEFIED_BIOGAS_FLUID_BLOCK.getDefaultState());
            world.setBlockState(pos.down(),ModBlocks.LIQUEFIED_BIOGAS_FLUID_BLOCK.getDefaultState());
            world.setBlockState(pos.down(2),ModBlocks.LIQUEFIED_BIOGAS_FLUID_BLOCK.getDefaultState());
            world.createExplosion(null, pos.getX(), pos.getY(), pos.getZ(),6.0f,true, World.ExplosionSourceType.BLOCK);
        }
        markDirty();
    }
