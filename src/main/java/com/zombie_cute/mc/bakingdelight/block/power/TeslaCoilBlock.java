...
public class TeslaCoilBlock extends BlockWithEntity implements Waterloggable {
    ...
    public void appendTooltip(ItemStack stack, @Nullable BlockView world, List<Text> tooltip, TooltipContext options) {
        if(Screen.hasShiftDown()){
            ...
            tooltip.add(Text.translatable(MiscUtil.TESLA_COIL_1).formatted(Formatting.GOLD));
            tooltip.add(Text.translatable(MiscUtil.TESLA_COIL_2).formatted(Formatting.GOLD));
            tooltip.add(Text.translatable(MiscUtil.TESLA_COIL_3).formatted(Formatting.GOLD));
            tooltip.add(Text.translatable(MiscUtil.TESLA_COIL_4).formatted(Formatting.GOLD));
            tooltip.add(Text.translatable(MiscUtil.TESLA_COIL_5).formatted(Formatting.GOLD));
        } else {
        ...
        if (MiscUtil.isCrowbar(player)){
            Direction dir = state.get(FACING);
             switch (dir){
                 case DOWN: {
                    switch (hit.getSide()){
                        case SOUTH -> world.setBlockState(pos,state.with(FACING,Direction.EAST));
                        case WEST -> world.setBlockState(pos,state.with(FACING,Direction.SOUTH));
                        case NORTH -> world.setBlockState(pos,state.with(FACING,Direction.WEST));
                        case EAST -> world.setBlockState(pos,state.with(FACING,Direction.NORTH));
                    }
                    break;
                }
                 case UP: {
                 ...
                 case EAST: {
                     switch (hit.getSide()){
                         case SOUTH -> world.setBlockState(pos,state.with(FACING,Direction.UP));
                         case UP -> world.setBlockState(pos,state.with(FACING,Direction.SOUTH));
                         case NORTH -> world.setBlockState(pos,state.with(FACING,Direction.DOWN));
                         case DOWN -> world.setBlockState(pos,state.with(FACING,Direction.NORTH));
                     }
                     break;
                 }
                 case WEST: {
                     switch (hit.getSide()){
                         case SOUTH -> world.setBlockState(pos,state.with(FACING,Direction.DOWN));
                         case UP -> world.setBlockState(pos,state.with(FACING,Direction.NORTH));
                         case NORTH -> world.setBlockState(pos,state.with(FACING,Direction.UP));
                         case DOWN -> world.setBlockState(pos,state.with(FACING,Direction.SOUTH));
                     }
                     break;
                 }
                 case SOUTH: {
                     switch (hit.getSide()){
                         case WEST -> world.setBlockState(pos,state.with(FACING,Direction.UP));
                         case UP -> world.setBlockState(pos,state.with(FACING,Direction.WEST));
                         case EAST -> world.setBlockState(pos,state.with(FACING,Direction.DOWN));
                         case DOWN -> world.setBlockState(pos,state.with(FACING,Direction.EAST));
                     }
                     break;
                 }
                 case NORTH: {
                     switch (hit.getSide()){
                         case WEST -> world.setBlockState(pos,state.with(FACING,Direction.DOWN));
                         case UP -> world.setBlockState(pos,state.with(FACING,Direction.EAST));
                         case EAST -> world.setBlockState(pos,state.with(FACING,Direction.UP));
                         case DOWN -> world.setBlockState(pos,state.with(FACING,Direction.WEST));
                     }
                     break;
                 }
            }
