...
public class FanBladeBlockEntity extends BlockEntity implements GeoBlockEntity {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, state -> {
            if (state.getAnimatable().getPos().getY() >= 200) {
                state.setControllerSpeed((float) state.getAnimatable().getPos().getY() / 100);
                if (Objects.requireNonNull(state.getAnimatable().getWorld()).isThundering()){
                    state.setControllerSpeed(5);
                } else if (Objects.requireNonNull(state.getAnimatable().getWorld()).isRaining()) {
