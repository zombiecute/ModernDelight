...
public class ModernDelightEMIPlugin implements EmiPlugin {
    ...
    public void register(EmiRegistry registry) {
        ...
        for (RecipeEntry<SteamingRecipe> recipe : manager.listAllOfType(SteamingRecipe.Type.INSTANCE)) {
            ...
            registry.addRecipe(new EMISteamingElectricRecipe(recipe.value()));
        }
        registry.addRecipe(new EMIAdvanceFurnaceRecipe());
        registry.addRecipe(new EMIOvenRecipe());
        for (RecipeEntry<SqueezeRecipe> recipe : manager.listAllOfType(SqueezeRecipe.Type.INSTANCE)) {
            registry.addRecipe(new EMIWoodenBasinRecipe(recipe.value()));
        }
        registry.addRecipe(new EMIIceCreamRecipe());
        for (RecipeEntry<JuiceExtractingRecipe> recipe : manager.listAllOfType(JuiceExtractingRecipe.Type.INSTANCE)) {
            registry.addRecipe(new EMIJuiceExtractingRecipe(recipe.value()));
        }
        registry.addRecipe(new EMIInstantNoodlesMakingRecipe());
        for (RecipeEntry<GrindingRecipe> recipe : manager.listAllOfType(GrindingRecipe.Type.INSTANCE)) {
            registry.addRecipe(new EMIGrindingRecipe(recipe.value()));
        }
    }
}
